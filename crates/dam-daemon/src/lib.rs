...
pub struct ProxyOptions {
    ...
    pub network_mode_explicit: bool,
    pub trust_mode: dam_trust::TrustMode,
    pub trust_mode_explicit: bool,
    pub target_name: String,
    pub provider: String,
    pub upstream: String,
    pub targets: Option<Vec<dam_config::ProxyTargetConfig>>,
    pub traffic_app_ids: Option<Vec<String>>,
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--network-mode" => {
                ...
                options.network_mode_explicit = true;
            }
            "--trust-mode" => {
                i += 1;
                options.trust_mode = required_value(&args, i, "--trust-mode")?.parse()?;
                options.trust_mode_explicit = true;
            }
            "--target-name" => {
                i += 1;
            ...
            "--target" => {
                i += 1;
                let target = parse_proxy_target_arg(required_value(&args, i, "--target")?)?;
                if options.targets.is_none() {
                    options.targets = Some(Vec::new());
                }
                if let Some(targets) = &mut options.targets {
                    targets.push(target);
                }
            }
            "--traffic-app" => {
                i += 1;
                let app_id = required_value(&args, i, "--traffic-app")?.trim();
                if app_id.is_empty() {
                    return Err("--traffic-app requires a non-empty app id".to_string());
                }
                options
                    .traffic_app_ids
...
pub fn proxy_config(options: &ProxyOptions) -> Result<dam_config::DamConfig, String> {
    let overrides = dam_config::ConfigOverrides {
        ...
        proxy_target_name: options
            .targets
            .is_none()
            .then(|| options.target_name.clone()),
        proxy_target_provider: options.targets.is_none().then(|| options.provider.clone()),
        proxy_target_upstream: options.targets.is_none().then(|| options.upstream.clone()),
        proxy_target_api_key_env: options.targets.is_none().then(String::new),
        proxy_targets: options.targets.clone(),
        traffic_enabled_app_ids: options.traffic_app_ids.clone(),
...
fn merge_traffic_route_proxy_targets(config: &mut dam_config::DamConfig) {
    for route in configured_traffic_routes(config) {
        if let Some(existing) = config.proxy.targets.iter_mut().find(|target| {
...
pub async fn serve_with_modes(
    ...
    let app = if network_mode == dam_net::CaptureMode::ExplicitProxy {
        Some(
            dam_proxy::build_app_with_interception(config.clone(), Some(interception.clone()))
                .map_err(DaemonError::BuildProxy)?,
        )
    } else {
        None
    };
    ...
    let result = if let Some(app) = app {
        ...
        dam_proxy::serve_transparent_with_shutdown(
            listener,
            config.clone(),
            interception,
            shutdown_signal(),
...
fn state_from_config(
    ...
    let mut trust = match trust_mode {
        dam_trust::TrustMode::Disabled => {
            ...
            if let Ok(Some(artifact)) = dam_trust::inspect_local_ca_artifact(&state_paths.state_dir)
            {
        ...
        dam_trust::TrustMode::LocalCa => {
            dam_trust::trust_state_for_state_dir(trust_mode, &state_paths.state_dir).map_err(
                |error| DaemonError::Message(format!("failed to inspect local trust: {error}")),
            )?
        }
    };
...
pub fn current_executable_identity() -> (Option<PathBuf>, Option<String>) {
    let Some(path) = current_executable_path() else {
        return (None, None);
    };
    let sha256 = executable_sha256(&path).ok();
    (Some(path), sha256)
}
...
pub fn executable_sha256(path: &Path) -> Result<String, std::io::Error> {
