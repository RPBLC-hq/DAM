...
fn usage_run() -> &'static str {
    "Usage: dam-daemon run [--config dam.toml] [--listen 127.0.0.1:7828] [--network-mode explicit_proxy|system_proxy|tun] [--trust-mode disabled|local_ca] [--target-name NAME] [--provider LABEL] [--upstream URL] [--target JSON] [--db vault.db] [--log log.db|--no-log] [--consent-db consent.db] [--resolve-inbound|--no-resolve-inbound]"
}
