...
fn usage() -> &'static str {
    "Usage: dam-proxy [--config dam.toml] [--listen 127.0.0.1:7828] --upstream URL [--target-name NAME] [--provider LABEL] [--failure-mode bypass_on_error|redact_only|block_on_error] [--resolve-inbound|--no-resolve-inbound] [--api-key-env NAME|--no-api-key-env] [--db vault.db] [--log log.db|--no-log]"
}
