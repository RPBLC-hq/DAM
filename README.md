...
- **Detection is intentionally narrow.** Email, NANP phone, US SSN, Luhn-validated
  cards, common API-key assignment labels, direct OpenAI/Anthropic/GitHub/Stripe/Slack/Google/AWS
  API-key families, Stripe webhook signing secrets, Slack incoming webhook URLs, PEM private key blocks, database connection URLs, and `Bearer`-labeled JWTs today. Names, addresses, unlabeled
