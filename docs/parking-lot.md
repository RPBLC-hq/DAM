...
Current state: the visible bundled app profile catalog is intentionally narrow: `claude` and `chatgpt` are available, and only those bundled MVP profiles are enabled by default when no user app-selection state exists. Imported/custom profile JSON remains loadable but opt-in until explicitly selected. The ChatGPT profile covers OpenAI API-key traffic, OpenAI platform traffic, ChatGPT subscription/login traffic, and Codex traffic through separate traffic app IDs. Generic OpenAI-compatible, generic Anthropic-compatible, xAI-compatible, and split Codex API/ChatGPT-login profiles are removed from the visible catalog for now. Existing local state that references retired profile IDs is normalized where possible so upgrades do not break the Settings or Connect views.
...
Current state: outbound requests are protected; bundled LLM traffic apps tokenize unknown outbound values, record those values in Activity, and do not write those detections to Wallet. Active Wallet consent lets matching provider-scoped values pass. Agent HTTP traffic apps resolve inbound DAM references and can opt into redetecting/redacting supported raw provider-returned values through traffic profile `inbound.protect_sensitive_data` without writing those inbound detections to Wallet. Domain-only values are not detected globally, but outbound email domains are retained as related context for opted-in inbound redaction. JSON-shaped responses are transformed string-by-string, including newline-delimited JSON, when reference restoration or explicit raw inbound protection is active. `text/event-stream` responses are transformed under the same route policy. Raw stream transformation handles references split across adjacent chunks; provider-aware SSE text-delta transformation handles references and opted-in raw values split across OpenAI-compatible and Anthropic JSON delta events with a bounded trailing event window instead of EOF buffering.
