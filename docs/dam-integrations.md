...
Connect app profiles are JSON files. Bundled profiles ship under `crates/dam-integrations/profiles/` and are seeded into `$DAM_STATE_DIR/integrations/profiles/<id>.json`. Runtime Settings and CLI lists are loaded from that same profile folder plus bundled JSON. A valid new profile JSON file in that exact folder appears as a profile without Rust code changes. Nested or hidden files outside that direct `profiles/*.json` catalog (for example legacy `profiles/<id>/latest.json` rollback state or hidden import scratch directories) are ignored and cannot make a custom profile visible or active by surprise. Bundled profile aliases are defined in JSON and normalize old IDs such as `codex` to the current profile ID during reads and migrations. Future imported or user-authored profiles use the same canonical state directory and one JSON file per profile, but the in-app profile creator/import/export workflow is parked. Adding a bundled profile is a data change: create a JSON profile with display metadata, connect args, optional explicit-proxy fallback settings, and `traffic_app_ids` that map to app IDs in the active `dam-net` traffic profile.
