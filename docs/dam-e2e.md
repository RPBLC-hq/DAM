...
The script builds `dam-proxy`, starts it on loopback with temporary vault/log SQLite files, and by default runs the bundled MVP HTTP route matrix: `openai-api` (`openai` target / `openai-compatible` provider), `anthropic-api` (`anthropic` target / `anthropic` provider), `claude-web` (`claude-web` target / `generic-http` provider), `anthropic-console` (`anthropic-console` target / `generic-http` provider), `claude-mcp-proxy` (`claude-mcp-proxy` target / `generic-http` provider), `claude-platform` (`claude-platform` target / `generic-http` provider), and `openai-platform` (`openai-platform` target / `generic-http` provider). For each route, it sends synthetic exact-echo and token-transformation requests plus the `agent_session_mixed_pii_secret_v1` realistic multi-line agent prompt/tool-output fixture containing synthetic email, phone, SSN-like, `.env` assignment, and assembled GitHub-shaped token values. It records the route ID, target name, provider, proxy `/health` target, observed detector kind/action counts, and `provider_forward_start` route lines in the JSON proof output, verifies exact echo resolution on the trusted client side, verifies a token-transformation prompt that asks the model to insert whitespace after reference opening brackets cannot reconstruct raw values, fails if the local activity log database contains any raw synthetic fixture values, scopes fake-upstream transcript assertions to that route when `GET /__dam/transcript` is available, asserts the upstream transcript saw DAM references/redactions for the agent fixture instead of raw fixture values, and removes the temp directory unless `--keep-temp` is passed. The proof output reports fixture name and raw-leak scan status without printing raw synthetic secret values. Exit code `2` means the local upstream or binary prerequisite is unavailable; exit code `1` means DAM failed the smoke check. Use repeated `--route <id>` flags to isolate one or more bundled HTTP routes while debugging; ChatGPT WebSocket routes stay on `scripts/dam-build.sh agent-websocket-smoke`.
