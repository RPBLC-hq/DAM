...
The Activity page polls this endpoint only while mounted and visible, refetches on window focus, and is invalidated by the existing local SSE stream when connect, wallet, or request state changes. Rows are detection/outcome log entries, not Wallet rows: ordinary Activity copy shows a safe local evidence label (`[email:<reference-id>]` when a reference exists, otherwise `[email]`-style kind labels), a compact outcome protection mark, the profile, and the actual token reference such as `[email:<reference-id>]` in the facts row when the log row carries one. Activity does not expose raw protected values by default in list, detail, or evidence JSON; intentional clear-value reveal stays in local Wallet/value-detail flows. Rows with a supported stored Activity value or vault-backed reference expose a one-way convenience action at `POST /api/v1/activity/:id/add-to-wallet`, so the UI can add the value to Wallet without first rendering the raw value in Activity. The Wallet remains a separate user-maintained list of preferred values for quick access and provider-scoped allows. Activity does not expose Wallet ids. The Activity allow-once action is parked and is not rendered in this slice.
