...
The Activity page polls this endpoint only while mounted and visible, refetches on window focus, and is invalidated by the existing local SSE stream when connect, wallet, or request state changes. Rows are detection/outcome log entries, not Wallet rows: they show the detected Activity value when the current log row carries one, falling back to a detected type placeholder such as `[email]`, a compact outcome protection mark, the profile, and the actual token reference such as `[email:<reference-id>]` when the log row carries one. Activity does not read Wallet values or expose Wallet ids. Rows with a stored Activity value can post that value to `POST /api/v1/wallet` as a one-way convenience action, but the Wallet remains a separate user-maintained list of preferred values for quick access and provider-scoped allows. The Activity allow-once action is parked and is not rendered in this slice.
