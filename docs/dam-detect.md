...
- Direct high-value secret families without assignment labels for the current MVP slice: OpenAI `sk-...` / `sk-proj-...` / `sk-svcacct-...`, Anthropic `sk-ant-...`, GitHub `ghp_` / `gho_` / `ghu_` / `ghs_` / `ghr_`, Stripe `sk_live_...` / `sk_test_...` / `rk_live_...` / `rk_test_...`, and Google API keys beginning with `AIza`.
...
Known current limitation: formats like `+1 (415) 555-2671`, zero-width-character obfuscation, and still-parked secret families such as bearer tokens, JWTs, webhook secrets, and other provider-specific formats not yet covered by the current regex set.
