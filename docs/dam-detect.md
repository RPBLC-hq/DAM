...
- Direct high-value secret families without assignment labels for the current MVP slice: OpenAI `sk-...` / `sk-proj-...` / `sk-svcacct-...`, Anthropic `sk-ant-...`, GitHub `ghp_` / `gho_` / `ghu_` / `ghs_` / `ghr_`, Stripe `sk_live_...` / `sk_test_...` / `rk_live_...` / `rk_test_...`, Google API keys beginning with `AIza`, and `Bearer`-labeled JWTs.
