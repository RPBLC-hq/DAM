...
- Direct high-value secret families without assignment labels for the current MVP slice: OpenAI `sk-...` / `sk-proj-...` / `sk-svcacct-...`, Anthropic `sk-ant-...`, GitHub `ghp_` / `gho_` / `ghu_` / `ghs_` / `ghr_`, Stripe `sk_live_...` / `sk_test_...` / `rk_live_...` / `rk_test_...`, Google API keys beginning with `AIza`, AWS access key IDs beginning with `AKIA` or `ASIA`, and `Bearer`-labeled JWTs.
...
Known current limitation: formats like `+1 (415) 555-2671`, zero-width-character obfuscation, and still-parked secret families such as unlabeled bearer tokens, webhook secrets, and other provider-specific formats not yet covered by the current regex set.
